...
- **Related Documents**: [sdd/requirement_qm.md](requirement_qm.md), [README.md](../README.md), [docs/architecture_agentic_ai.html](../docs/architecture_agentic_ai.html), [docs/ROUTER_REDESIGN.md](../docs/ROUTER_REDESIGN.md), [docs/MEMORY_CONTEXT_REDESIGN.md](../docs/MEMORY_CONTEXT_REDESIGN.md), [docs/DIALOGUE_STATE_REDESIGN.md](../docs/DIALOGUE_STATE_REDESIGN.md)

## Version History

### v1.31 — 2026-08-30 (implementation complete)
Carries forward `requirement_qm.md` v1.31 (Requirement 3 AC3 corrected, AC11 strengthened; Requirement 4 AC2 strengthened). Traced via the live query log across two separate days (`query_log_2026-08-25.txt`, `query_log_2026-08-28.txt`) to the same misroute pattern recurring in unrelated conversations — a short, topic-less reply continuing a course enquiry ("Ya, how about course 2") repeatedly misclassified ENROLLMENT. See Component 3 for the classifier-context and sticky-dispatch mechanism (the actual fix), and Component 4 for an unrelated `preferred_course`-overwrite bug found while reviewing the same code path. Full implementation record lives in `docs/MEMORY_CONTEXT_REDESIGN.md` and `docs/DIALOGUE_STATE_REDESIGN.md` (Phases 0–4 complete; Phases 5–6 — full digression handling, retiring the remaining keyword overrides — not started) rather than `task_qm.md`, which has no corresponding task entry for this work.
...
- **Enrolment-signal keyword too narrow — "attend" not recognised (Requirement 3 AC5, v1.29)**: reported live alongside the data-loss bug detailed in Component 4 — "I want to attend Infection Control for Dental Clinics" (clear enrolment intent) had no explicit signal per `_ENROLLMENT_KEYWORD_RE`, so the pre-existing reminder-in-progress override (predates v1.26) forced it back to Module A instead of triggering Module B's registration gate. Widened to `enrol\w*|enroll\w*|sign\s*up|register|join\s+the\s+course|attend\w*` — also fixes a separate, previously-unnoticed gap in the same regex where only the bare verb forms "enrol"/"enroll" matched, so "enrolling"/"enrolled" never did either. Verified live: "I want to attend..." now correctly reaches Module B's registration gate.
- **Classifier context replaced with a participant-only, session-scoped window (Requirement 3 AC3, v1.31, `docs/MEMORY_CONTEXT_REDESIGN.md` Phases 2–2.1)**: traced via the live query log to a recurring misroute — "Ya, how about course 2" / "what course 2" / "no. How about course 2", each a topic-less continuation of a prior course discussion, repeatedly misclassified ENROLLMENT. Root cause: `classify_free_text()`'s prompt included the last 6 chat turns' raw text, both roles, with no bound on how far back "recent" reached — a message with no topic of its own has nothing reliable to anchor against once the assistant's own wording is mixed in unbounded. `_recent_history()` (still `repo.recent_memory(wid, limit=6, session_id=...)`, feeding the unchanged AC5/AC7/AC8 deterministic overrides) is now scoped to `repositories.current_session_id(wid)` — a fresh session begins after 60 minutes of participant inactivity. The classifier prompt itself no longer sees any raw assistant text at all: it's built from only `participant_turns[-3:]` (the participant's own last 3 messages) plus one new fixed-vocabulary line from `_assistant_last_action()` (`listed_courses`, `asked_which_intake_for_reminder`, etc. — a closed label set, not free text) describing what the assistant's most recent turn did. Verified against 9 historical misroute cases replayed live through the current router (`backend/evals/replay_router_misroutes.py`, a new permanent regression harness against the real logs): the session-scoped participant-only window alone fixed 5/9; the remaining 4 (a short "how about course N" with only 1–2 prior participant turns) needed `_assistant_last_action()` restored in this narrower, structured form before all 9 passed — stripping assistant text entirely had overcorrected.
- **`_restates_pending_course()` — a pending flow's own code no longer counts as a switch signal (Requirement 3 AC11, v1.31, `docs/MEMORY_CONTEXT_REDESIGN.md` Phase 3)**: `_explicit_module_signal()` previously treated ANY course/schedule code match (`_CODE_RE`) as an unconditional signal to break out of Tier 0 sticky dispatch into Tier 1 classification — including a participant simply repeating the course already under discussion (e.g. restating "C2601" mid-reminder for that same course), forcing an unnecessary classification round-trip for a message that wasn't actually asking to switch anything. New `_restates_pending_course(body, state)` checks the matched code against the pending flow's own `conversation_state.flow_context` (`course_code`/`schedule_code`/`candidates` — dual-written by Module A/B since v1.28's Phase 1, but never read back to affect a routing decision until now); a code only counts as an explicit signal when it names something genuinely different from what's already pending. Falls back to the old unconditional behaviour whenever `flow_context` is empty, so a flow that hasn't been touched since before v1.28 cannot regress.
- **`TurnResult`/centralized `conversation_state` writes (v1.31, `docs/DIALOGUE_STATE_REDESIGN.md` Phase 4) — deliberately not a behaviour change**: `module_a.py`/`module_b.py` no longer call `repositories.set_conversation_state()`/`clear_conversation_state_if_owner()` directly (their v1.26 write points, described above, are unchanged in *when* they fire — only *where*); each now returns a `TurnResult(reply, module, pending, slots)` and `orchestrator.py: _apply_turn_result()` performs the single write. Decision logic is byte-for-byte unchanged, confirmed by the doc's own phase-4 regression pass; this is purely what makes Phase 3's `flow_context` read (above) possible, by giving both modules a single, structured place to report their resolved slots from.

---

### Component 4: Module A — Enquiry & Lead Agent (`backend/app/crews/module_a.py`)
...
- **An incidental course lookup mid-reminder overwrote the reminder's own course (Requirement 4 AC2, v1.31)**: found while reviewing the same code path as Component 3's v1.31 fix — a participant idly asking about a different course's fees or schedule while a reminder for an earlier course was still pending (e.g. checking course 3's dates mid-reminder for course 1) silently overwrote `customers.preferred_course` via `tools/course_tools.py: _remember_interest()`, the secondary capture net (v1.7) that fires whenever `Course Fees`/`Course Schedule` is actually called. The reminder's own later "which intake?" question then resolved against the wrong course. Fixed by gating `_remember_interest()` on `conversation_state`: when `active_module == "A"` and `active_flow == "awaiting_reminder"` for this participant, the incidental lookup's course is not written to `preferred_course` — the reminder flow's own course stays authoritative until it resolves or clears. This is the first tools-layer read of `conversation_state`; every other consumer to date has been router.py or an agent module directly.
